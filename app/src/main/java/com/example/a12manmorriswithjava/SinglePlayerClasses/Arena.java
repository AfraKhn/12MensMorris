...
public class Arena implements Cloneable {
    ...
    boolean putTaw(Taw taw, int index) {
        try {
            ...
            if (checkScored(index)) {
                ...
                opponent.setTawsOnHand(opponent.getTawsOnHand() - 1);
            } else {
                changeTurn();
            }
            return true;
...
//        };
    ...
    private void examine1(Arena arena, Map<Integer, Double> statistics) {
        Taw turn = arena.turn;
        for (int i = 0; i < 24; i++) {
    ...
    private void examine2(Arena arena, Map<Integer, Double> statistics) {
        Taw turn = arena.turn;
    ...
    private void examine3(Arena arena, Map<Integer, Double> statistics) {
        Taw turn = arena.turn;
        for (int i1 = 0; i1 < 24; i1++) {
            double sum = 0;
            int gameRounds = 0;
    ...
    private void moveSimulator(Arena arena, Map<int[], Double> statistics, int index) {
        Player opponent = arena.getPlayers().get(arena.getTurn() == Taw.RED ? Taw.BLUE : Taw.RED);
        if (opponent.getCatchedTaws() < 9) {
            findTargetsInMove(arena, index, statistics);
        } else {
            findTargetsInFly(arena, index, statistics);
        }
    }

    private void findTargetsInMove(Arena arena, int index, Map<int[], Double> statistics) {
        Taw turn = arena.turn;
        for (int i = 0; i < 8; i++) {
            int target = -1;
            switch (i) {
            ...
            if (target != -1) {
                arena.move(index, target);
                if (checkScored(target)) {
                    findRemovableNodes(arena, index, target, statistics);
                } else {
                    int[] result = {index, target, -1};
                    statistics.put(result, (double) analyze(arena, turn));
                }
            }
        }
    }

    private void findTargetsInFly(Arena arena, int index, Map<int[], Double> statistics) {
        Taw turn = arena.turn;
        for (int i = 0; i < 24; i++) {
            if (arena.nodes[i].getTaw() == null) {
                arena.move(index, i);
                if (checkScored(i)) {
                    findRemovableNodes(arena, index, i, statistics);
                } else {
                    int[] result = {index, i, -1};
                    statistics.put(result, (double) analyze(arena, turn));
                }
            }
        }
    ...
    private void findRemovableNodes(Arena arena, int index, int target, Map<int[], Double> statistics) {
        Taw turn = arena.turn;
        for (int i = 0; i < 24; i++) {
            if (arena.nodes[i].getTaw() == turn) {
                arena.del(i);
                int[] result = {index, target, i};
                statistics.put(result, (double) analyze(arena, turn == Taw.BLUE ? Taw.RED : Taw.BLUE));
            }
        }
    }
}
